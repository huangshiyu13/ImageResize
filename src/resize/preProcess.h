...
void costDW(IplImage* Img, int W , int H, CutPath &cutPath){
	//cout << "inin2"<<endl;
	for (int x = 0; x < W; ++x)
		for (int y = 0; y < H; ++y){
...
void costDH(IplImage* &Img, int W , int H, CutPath &cutPath){
	for (int x = 0; x < W; ++x)
		for (int y = 0; y < H; ++y){
			myCV2D[x][y] = cvGet2D(Img, y, x);
		}
...
void preProcessW(FWH* &fwh, IplImage* oriImg, CvSize size){
	int oriW = size.width;
	...
	for (int j = oriW-1 ; j >= 1 ; j--){
		FWH &fwht = fwh[j];
		costDW(img, j+1 , oriH ,fwht.cutPath );
	}
	cvReleaseImage(&img);
}

void preProcessH(FWH* &fwh, IplImage* oriImg, CvSize size){
	int oriW = size.width;
	int oriH = size.height;
	...
	for (int j = oriH-1 ; j >= 1 ; j--){
		FWH &fwht = fwh[j];
		costDH(img, oriW, j+1 ,fwht.cutPath );
	}
	cvReleaseImage(&img);
}
